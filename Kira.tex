...
%However, the workflow approach seeks to preserve the original program thus limits the programming flexibility. 
%The Hadoop approach mandates the data flow of the application, and may introduce unnecessary I/O. 
%While the MPI approach has to introduce application specific data management scheme to make better use of the I/O system.

%Briefly talk about existing approaches to enable many task applications
%The tradeoffs here are performance, flexibility, optimization generality:
%\begin{itemize}
%\item[] 1. Workflow -- process level parallelism, mid performance, worst flexibility, best generality
%\item[] 2. Integrate as a single MPI program -- thread level parallelism, best performance, best flexibility, worst generality
%\item[] 3. Kira, integrates at library level -- worst performance, best flexibility, best generality
%\end{itemize}
%Talk about real performance. 
...
%driven by 
made through the collection and analysis of large datasets.
...
Research groups frequently start with a C or Fortran program that is optimized for processing a small amount of data on a single-node workstation and then use distributed processing frameworks to improve processing capacity.
...
When building large many-task scientific applications, \linebreak there is a challenging tradeoff between programmability, performance, and fault-tolerance. 
Scientists have used dedicated workflow systems (Galaxy~\cite{goecks10}, HTCondor~\cite{litzkow88}, and Swift~\cite{wilde11}) or parallel frameworks like the message passing interface (MPI, \cite{gropp96}) to build these applications.
However, workflow systems sacrifice programming flexibility by running a single-process program as a monolithic entity.
While MPI-based approaches enable fast execution and good scalability, MPI does not support fault tolerance, which makes applications fragile. Additionally, MPI-based solutions must employ an application specific data management scheme.
With the rise of the Hadoop~\cite{HADOOP} ecosystem, recent work~\cite{ekanayake08, ekanayake10} has evaluated whether Hadoop can support many-task applications.
Although Hadoop's map-reduce programming model can be used to express many-task applications, since Hadoop MapReduce only supports an all-to-all shuffle, it is cumbersome
to support a wide variety of dataflow patterns in Hadoop MapReduce. Additionally, Hadoop MapReduce's dependence on shuffling data to disk makes it a bad fit for iterative and pipelined tasks~\cite{ekanayake10}.


%Traditionally, dedicated workflow systems and Message Passing Interface~(MPI) libraries have been used to construct these many-task applications.
%With the rise of the Hadoop~\cite{HADOOP} ecosystem, recent work~\cite{ekanayake08, ekanayake10}
%\franknote{I've got a few other works to cite here} has evaluated Hadoop's ability to support multi-task scientific workflows.
%\zhaonote{Can we put them in the related work section?}

%Workflow systems, such as HTCondor~\cite{litzkow88}, Galaxy~\cite{goecks10}, and Swift~\cite{wilde11} can run single-process programs across a parallel environment.
%These systems explicitly connect stages into a dataflow. However, parallelizing at the application level reduces the flexibility of the original program. For example, although the SExtractor application~\cite{bertin96} is written as a monolithic C program to extract astronomical objects from images, the application's logic allows background estimation, background removal, object detection, and astrometric and photometric estimation to be separated into distinct stages. The instrumentation of astrometric and photometric parameters are specified in configuration files which are not trivial for users to understand.
%\franknote{Not sure what the purpose of this sentence is? I don't think we tackle the ``application configuration'' problem in this paper.}
%\zhaonote{Yes, we should remove this sentence. Please go ahead.}
%The SExtractor application can also be used to remove previously detected objects from an original image. This process can be performed iteratively to improve the final object extraction. Although the C program provides all the required semantics for building this iterative process, it does not expose the interfaces through the command line. This prevents workflow system based approaches from exposing these interfaces to users.
%\franknote{Is the implication here that this prevents people from running the object extraction stages iteratively in a workflow system?}
%\zhaonote{yes, that's what exactly I mean.}

%MPI~\cite{gropp96} is another tool used to implement parallel task execution on supercomputers. mpiBLAST~\cite{lin08} is an example application built using MPI as the communication protocol for connecting the execution of single process programs (BLAST~\cite{altschul90}). mpiBLAST's customized data management scheme enables fast execution with good scalability. However, the dataflow implemented in an MPI approach is application specific and can not be reused for other applications.

Apache Spark~\cite{zaharia12, zaharia10} emerged as a dataflow-based distributed computing system
...
One extra advantage of Spark is that it is actively maintained by a large group of open source developers and 
is unlikely to be impacted by the ebbs and flows of academic funding.

%We provide a comprehensive study of the commodity open-source Apache Spark system's capability of supporting many-task applications
%and demonstrate that Apache Spark system provides primitives that are sufficient for implementing
%arbitrary dataflows. To improve the efficiency and generality of task parallel applications, we then evaluate the tradeoffs between
%building a custom, application-specific workflow, versus using a commodity task-parallel workflow system. 


%To evaluate this, we compare the Kira SE application against an optimized, C-based Source Extractor
%implementation, and demonstrate that our proposed approach achieves better scalability than the C-based approach, while maintaining
%generality. The Kira application is open source, and is designed to be easily extended by astronomers\footnote{Kira is available from
%\url{https://github.com/BIDS/Kira} and is licensed under the non-viral MIT license.
%\franknote{Why is Kira MIT licensed, BTW? Typically, we've used the Apache 2 license. Is this because of the dependency on the GPL'ed jFITS library?}}.
%\zhaonote{I actually haven't decided which license to use. I will rewriting the code with pySpark before release v0.1, so in that case, we won't rely on JFITS library which has a GPL license. The MIT license is randomly chosen.}
...
Kira can process data stored on the Hadoop Distributed File System~(HDFS, \cite{shvachko10}) and shared file systems such as GlusterFS~\cite{davies13}, Lustre~\cite{donovan03}, and GPFS~\cite{schmuck02}.
...
Kira SE runs 3.7x faster than running the C program in parallel in the EC2 cloud on the 1TB Data Release 7 from the Sloan Digital Sky Survey~\cite{abazajian09, york00}.
...
the Panoramic Survey Telescope \& Rapid Response System (PanSTARRS)~\cite{kaiser02},
...
build new applications based on SExtractor's primitives. Kira integrates with the SEP
library and benefits from SEP's reorganization of SExtractor.
...
image~\cite{LSST-link}. This telescope produces approximately 12.8~GB data in 39~seconds
...
oriented API~\cite{zaharia12, zaharia10}. \linebreak Spark's development was motivated by a need for a
...
Spark provides Scala, Java, and Python programming interfaces, within a functional
programming paradigm. Spark usually uses HDFS~\cite{HDFS} for persistent storage, but
Spark can process data stored in S3 or a shared file system such as
GlusterFS~\cite{davies13} or Lustre~\cite{donovan03}. Spark tracks job lineage, which is used for scheduling the
recomputation of data when a node fails. Spark has been extended to support a variety of
...
DryadLINQ~\cite{yu08} and Spark~\cite{zaharia10} allow for applications to be decomposed into
...
by re-executing the section of the DAG that computed the lost partition~\cite{zaharia12}. In
future work, we plan to examine how Spark's lineage information can be used in scientific
applications to enable the design of reproducible scientific pipelines.
...
given in \S~\ref{sec:Programming}. An additional benefit of integrating at the library
...
parallel into tuples that users can transform and compute upon. In future work we plan to evaluate
various other data formats (e.g. Avro~\cite{avro}, Parquet~\cite{parquet}, and HDF5~\cite{folk99})
with Kira.
...
we configured Spark to support process level level parallelism by launching a worker instance for each
...
Software configurations are described in each of the following experiments.
...
\begin{figure}[h]
	\begin{center}
		...
		\caption{Scale-Out Performance Comparison between Kira SE and the C Version in Logarithmic Scale (Lower is Bßetter)
		\label{fig:scaleout}}
  	\end{center}
\end{figure}
...
We configure GlusterFS in \emph{replicated} and \emph{distributed} \linebreak modes and compare Kira
...
The C version is slower for several possible reasons:
...
\begin{enumerate}
\item The C version is executed in parallel in a batch mode where node starvation can occur
at the end of a run.
\item The metadata overhead of processing 176,938 files in a single directory is too large.
\end{enumerate}
...
supercomputers outperform commodity clusters for data-intensive applications~\cite{jha14}.
...
across large compute clusters. This includes workflow systems such as HTCondor~\cite{litzkow88},
Cactus~\cite{allen01}, Pegasus~\cite{deelman04}, Galaxy~\cite{goecks10} and Swift~\cite{wilde11},
...
Researchers have used the Hadoop MapReduce~\cite{HADOOP} system to paralleize
tasks using a map-reduce data model. A variety of applications have been paralleized
using Hadoop, including SCIMP~\cite{ekanayake08}, CloudBLAST~\cite{matsunaga08} and
MARISSA~\cite{dede12}. Several applications have been easily enabled through the use
of the Hadoop Streaming API~\cite{leo09}. Although Hadoop exposes many convenient
abstractions, it is difficult to target the restrictive map-reduce API~\cite{dewitt08}
and Hadoop's disk based model makes iterative/pipelined tasks expensive~\cite{ekanayake10}.
...
mosaicing applications (Montage, \cite{jacob09}), protein docking simulators (DOCK6, \cite{lang09}), and
sequence alignment and search toolkits(BLAST, \cite{altschul90}) applications. As an
...
communication stages are critical for performance~\cite{lin08}, which means that the
...
Shared file systems (e.g. GPFS~\cite{schmuck02}, Lustre~\cite{donovan03}, and
...
do not expose file locality to workflow systems, thus makaking suboptimal use of local
disks on the compute nodes when possible. Most tools in the Hadoop ecosystem use
HDFS~\cite{HDFS, shvachko10}, which is based off of the Google File System~(GFS,
\cite{ghemawat03}). HDFS and GFS both provide a shared namespace, but are not POSIX
compliant. Unlike traditional server-based shared file systems, HDFS and GFS use
...
ecosystem when running embarassingly parallel data intensive scientific applications. This is
...
We also demonstrated that Spark provides API semantics that are sufficient to support many-task
applications. The task model, support for common dataflows, and built-in support for fault
