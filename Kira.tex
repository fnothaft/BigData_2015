...
This section discusses the Spark configuration tuning in parallelism and scheduling to make
...
wait for a short period time~\cite{ananthanarayanan11}. By sweeping the waiting time, we found that waiting for 0ms delivered the best performance.
While this result seems to contradict our result in~\S\ref{sec:Performance} that states that Apache Spark outperforms
the HPC solution due to data locality, it is not contradictory because eliminating the delay scheduling penalty still leads to
98\% of tasks being scheduled with data locality. We have traded 2\% worse locality for a 4.5\% improvement in task balance.
...
The C version in \emph{replicated} slows down $2.8\times$ than that in \emph{distributed} mode because the directory metadata query
is dramatically slower ($13.5\times$), and the additional replica for each output file and associated metadata update introduces a slowdown
of $2.2\times$.
...
that causes the performance variance. In Figure~\ref{fig:1tb-edison}, we use the idle mode to refer to the 
...
We believe this is important, because leveraging the Apache Spark platform would enable scientists to benefit from the rapid pace of innovation and large range of systems and technologies that are being driven by wide-spread interest in Big Data analytics.
