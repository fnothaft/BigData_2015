...
To run the C version in parallel, we launch a number of processes in the background and then wait
...
We believe that this problem is caused by sharing the single Aries network between both inter-process
communication and I/O. The network resource is not fairly shared to preserve every
